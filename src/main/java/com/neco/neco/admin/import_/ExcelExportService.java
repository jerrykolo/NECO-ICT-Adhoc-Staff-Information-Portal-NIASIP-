...
public class ExcelExportService {
    ...
    private byte[] generateStaffWorkbook(List<Staff> staffList) throws IOException {
        ...
        String[] columns = {"Staff ID", "Full Name", "Designation", "Department",
                "Division", "Role", "Category", "Phone", "Email", "Status"};
        ...
        for (Staff staff : staffList) {
            ...
            row.createCell(6).setCellValue(staff.getCategory() != null ? staff.getCategory() : "");
            row.createCell(7).setCellValue(staff.getPhoneNumber() != null ? staff.getPhoneNumber() : "");
            row.createCell(8).setCellValue(staff.getEmail() != null ? staff.getEmail() : "");
            row.createCell(9).setCellValue(staff.getAccountStatus().name());
        }
